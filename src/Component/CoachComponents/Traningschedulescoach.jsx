...
const Traningschedulescoach = ({user}) => {
    ...
    const fectdata = async () => {
        try {
            const data = await axios.get(`http://localhost:8056/viewDataTraningplan/${user.coachid}`);
            // alert('datafatch');
   ...
   const deletestudent = async (id) => {
        ...
        if (window.confirm('Are your Sure???')) {
            try {
                const del = await axios.delete(`http://localhost:8056/viewDataTraningplan/${id}`);
                if (del.data) {
                    // alert('deleted data');
                    fectdata();
     ...
     const formedit = async (e) => {
        e.preventDefault();
       try {
         await axios.put(`http://localhost:8056/viewDataTraningplan/${edit.planid}`,edit);
...
setWorkdata({
    ...
    const workdrilladd=async(e)=>{
        e.preventDefault();
         try {
                const del = await axios.post(`http://localhost:8056/addwork/${workdata.planid}`,workdata);
                if (del.data) {
                    alert('DRIL IS ADD');
                    setWorkdata(null);
